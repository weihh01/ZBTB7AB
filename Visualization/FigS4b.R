...
aaa$type <- factor(aaa$type, levels = c("PP","EP"), labels = c("Pro-Pro","Enh-Pro"))


ggplot(data = aaa, mapping = aes(x = dataset, y = ratio, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 0.85) +
  scale_fill_manual(name = "", values = c("#F1B656", "#397FC7")) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 0.8), breaks = seq(0, 0.8, 0.2), labels = seq(0, 0.8, 0.2) * 100) +
