...
library(openxlsx)
library(reshape2)
aaa <- read.xlsx("motif.xlsx")

colnames(aaa) <- c("type", "ratio", "dataset")
aaa$type <- factor(aaa$type, levels = c("one motif", "two motif", "two plus motif"), labels = c("1 motif", "2 motif", "2+ motif"))
...
ggplot(data = aaa, mapping = aes(x = dataset, y = ratio, fill = type)) +
  ...
  scale_fill_manual(name = "", values = c("#4E79A7","#59A14F","#F28E2B")) +
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1), breaks = seq(0, 1, 0.25), labels = seq(0, 1, 0.25) * 100) +
  theme(legend.position = "right", legend.title = element_text(size = 14), legend.text = element_text(size = 13))

ggsave(filename = "motif.pdf", width = 4.5, height = 3.3, dpi = 600)
