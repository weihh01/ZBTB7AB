...
aaa$type <- factor(aaa$type, levels = c("PP","EP","EE","struc","others"), labels = c("Pro-Pro","Enh-Pro","Enh-Enh","Struc-Struc","others"))

ggplot(data = aaa, mapping = aes(x = dataset, y = ratio, fill = type)) +
  geom_bar(stat = "identity", position = "stack", width = 0.85) +
  scale_fill_manual(name = "", values = c("#F1B656", "#397FC7", "#6F6F6F","#4CAF50","#B07CC6")) +
...
ggsave(filename = "EP.pdf", width = 3.5, height = 4, dpi = 600)
